...
"%JAVA_HOME%\bin\java" -Xms128M -Xmx1024M -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005  -Dfile.encoding=UTF-8 -Dsolr.solr.home=specialpurpose/solr -jar ofbiz.jar > runtime\logs\console.log

rem ### Simple easy to read line
echo on
rem "%JAVA_HOME%\bin\java" -Xms128M -Xmx1024M -XX:MaxPermSize=512m -Dfile.encoding=UTF-8 -Dsolr.solr.home=specialpurpose/solr -jar ofbiz.jar
echo off
rem ### If you would prefer the console output to be logged rather than displayed switch out the above line for this one
rem "%JAVA_HOME%\bin\java" -Xms128M -Xmx1024M -XX:MaxPermSize=512m -Dfile.encoding=UTF-8 -Dsolr.solr.home=specialpurpose/solr -jar ofbiz.jar > runtime\logs\console.log
